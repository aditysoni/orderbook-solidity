...
contract OrderbookFactory {
    ...
    event OrderbookCreated(bytes32 indexed marketHash, address orderbook, string baseToken, string quoteToken);

    function createOrderbook(string memory baseToken, string memory quoteToken) external returns (address) {
